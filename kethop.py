...
for i in range(len(x_test)):
    svm_prediction = y_pred[i]
    knn_prediction = yknn_pre[i] 
    #if svm_prediction == knn_prediction:
    combined_predictions.append(svm_prediction) 
...
def dudoan():
    ...
    yknn_pre=knn_model.predict(tranf)
...
def tile():
    ac=accuracy_score(y_test,combined_predictions)*100
    av= round(ac,4)
    label_ketqua = Label(new,font=('Arial',12),text='Tỉ lệ chính xác: '+ str(av) +' % ',fg='red')
...
nut_dd= Button(new,text='Tỉ lệ chính xác',width=10,height=2,command=tile)
